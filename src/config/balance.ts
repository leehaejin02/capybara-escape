 ...
 *
 * ── 검증 상태 ───────────────────────────────────────────────────────────
 * ⚠️ **아직 어떤 값도 "합격"이 아니다.** (2026-08-07, playtest 1차 측정 후)
 * 1차 측정(6시드 × 200회 = 1200회): 클리어율 7.0% / 잔여 39.4s / 피격 2.84 / timeout 1.4%
 * → **4개 지표 전면 미달.** 단일 원인은 치사율 과다(고블린 접촉)로 특정됐다.
 * 이 파일의 값 중 **실측 근거를 가진 것은 `GOBLIN.CHASE_SPEED_PX_PER_SEC` 하나뿐**이고,
 * 그것도 "미달을 고치려는 2라운드 가설"이지 합격값이 아니다.
 * 조정은 **한 번에 한 축만** 한다. 라운드 계획은 docs/DECISIONS.md D7에 있다.
...
export const GOBLIN = {
   ...
   *   폭이 큰(-20) 이유: playtest가 85를 이미 실측했고(클리어율 11.3%, 여전히 미달) 5px 단위
   *   탐색으로는 남은 마감 안에 35%까지 못 간다. **구간을 좁히려고 일부러 크게 넘긴다** —
   *   55%를 넘겨도 그건 실패가 아니라 상한 확보이고, 다음 라운드에 83 부근으로 이분한다.
   */
  CHASE_SPEED_PX_PER_SEC: 75,
  /**
   * 시야 부채꼴 반경 (px). GDD 6장.
   * ⚠️ **다음 라운드(3) 1순위 후보이지만 이번 라운드에는 건드리지 않는다.**
   * playtest 단독 실험 실측: 140 → 100에서 클리어율 7.0% → 12.9%(여전히 미달).
   * CHASE_SPEED와 같은 라운드에 움직이면 어느 쪽이 효과였는지 영원히 알 수 없다.
   */
