...
// tech 수정 (2026-08-07, 사용자 피드백 — ART.md §3.3 tech 구현 노트):
//  (a) 뾰족귀를 실루엣 위(y10..11)로 2px 돌출하는 스파이크로 올림 — 원안(y12..13)은 등 상단
//      외곽선에 흡수돼 측면에서 귀가 사라졌다.
//  (b) 목 경계 ink 세로선 (17,15)-(17,21) 추가 — 머리(x17..24)와 몸통(x9..17)이 병합돼
//      내부 외곽선이 없어 한 덩어리로 읽혔다.
//  (c) 허리띠 7×2 → 5×2 (x11..15) — 원안은 실루엣 내부 전 폭을 가로질러 형태를 두 동강 냈다.
const rightShapes = [
  { x0: 9, y0: 12, x1: 17, y1: 23 }, // 등·몸통 9×12 — top y=12 (스파이크 제외 실루엣 최상단)
  { x0: 17, y0: 14, x1: 24, y1: 22 }, // 머리 8×9 — 앞으로 처진 머리
  { type: 'points', points: [[18, 13], [19, 13], [19, 12], [20, 12], [20, 11], [21, 11], [21, 10]] }, // 뾰족귀 스파이크
...
const rightDetails = [
  ...
  ...pts(17, 15, 17, 21, INK), // 목 경계선 — 머리/몸통 분리 (tech 수정 b)
  ...pts(11, 19, 15, 20, BELT), // 허리띠 5×2 (tech 수정 c)
];
