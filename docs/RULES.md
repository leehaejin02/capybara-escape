...
- 내보낼 것: `MAP_ROWS: readonly string[]` (아래 원본), 그리고 이를 1회 파싱해 만든
  `isSolid(col, row): boolean`, `MISSION_POINTS`, `GOBLIN_ROUTES`, `PLAYER_START`, `EXIT_POINT`.
- 파싱은 **모듈 로드 시 1회**. 매 틱 문자열을 인덱싱하지 않는다 (성능이 아니라 실수 방지 목적).
...
이 격자는 다음 성질을 **맵 데이터 수준에서 보장**한다 (gd가 수동 검증했다):
...
> 맵을 수정하면 반드시 이 4개 열 + 3개 행에 `#`이 들어가지 않았는지 재검사한다.
> `verify`가 기계적으로 검사할 수 있는 조항이다 (문자열 인덱싱 21회).
...
> 이건 **의도적으로 어렵게 잡은 초기값**이다. 클리어율이 35% 아래로 나오면 `playtest`의 1순위 조정 대상은
> 이 순찰로가 아니라 `GOBLIN.VISION_RADIUS_PX` 또는 `GOBLIN.PATROL_SPEED_PX_PER_SEC`다.
> **순찰로는 맵 데이터이므로 밸런스 축이 아니다** — 바꾸려면 이 문서를 고쳐야 한다.
